...
**Why:** Real-device testing produced constant scan failures. Root cause: modern phone cameras shoot 8 to 12 MB photos, which exceeded the vision API's payload limit, the most valuable bug of the project because it only appears with real hardware. The web-search feature came from a real user need: when the model isn't sure, it should be able to look it up rather than guess.
...
**Why:** Everything before this milestone was verified against mocks. A real key and real images are where the remaining assumptions break, and they did, in three ways.
...
**Next:** Deploy the backend to Render and the frontend to Vercel per the deployment guide, then test camera scans and clipboard-paste scans on real devices against the live URL.
...
One last bug in the family was the strangest: deep into a conversation Berry started replying "Recipe given: X" as plain text, with an empty recipe field, then insisted the recipe was "in the recipe panel" (no such panel exists). The cause was self-imitation: the frontend replayed his past structured turns to the model as prose shorthand ("Dishes offered: ...", "Recipe given: X"), so after a few turns the transcript taught him that shorthand was the expected answer format. Past turns are now replayed as the exact JSON he produced, every in-context example reinforces the contract, and the prompt names the anti-pattern and orders a full resend whenever a user says they cannot see a recipe. Verified against production: the failing conversation now returns the complete recipe object.

**Next:** Streaming responses, saved scan history, and README screenshots of the current design.

## Milestone 8: A secure baseline

**Built:** A security hardening pass across the proxy. Rate limiting grew from a single 20/hour per-IP cap into two layered windows, a burst window (5/minute) that catches rapid-fire abuse and the sustained hourly window for longer runs; whichever trips first now returns 429 with a `Retry-After` header telling the client exactly how long to wait, and a blocked caller's hit is no longer recorded so retries cannot push them deeper. On top of that sits a service-wide daily budget (default 1000 upstream calls per UTC day): because the model API has no per-request cost ceiling, one persistent caller staying under the rate limit could still run up a bill, so once the day's budget is spent both endpoints return 503 with a `Retry-After` pointing at the next UTC midnight. The budget is charged only on a real upstream call, so rejected input never burns it. Every API response now carries security headers (HSTS, nosniff, X-Frame-Options DENY, Referrer-Policy, a locked-down Permissions-Policy, and a `default-src 'none'` CSP since the API only emits JSON), including error responses, and CORS was narrowed from `allow_headers=["*"]` to just `Content-Type`. The frontend gained a strict enforced CSP and matching headers via `vercel.json`, with the camera allowed (the PWA needs it) but microphone and geolocation denied. A new `SECURITY.md` documents the posture, how to report a vulnerability, and operational guidance; `.env.example` documents every new tunable. Backend suite grew from 52 to 60 tests.

**Why:** The app was already sound on secrets (key server-side only, `.env` gitignored and never committed, git history clean) and on input validation, but it had no defense-in-depth against cost abuse and no transport hardening beyond what the platforms gave for free. A public AI endpoint without a spending cap is a standing liability.

**Problems solved along the way:** Adding a 5/minute burst limit immediately broke test isolation, since the whole suite hammers the same in-process limiter from one client; an autouse fixture that resets the limiter and budget between tests fixed it and made the older manual `_hits.clear()` calls redundant. The two rate-limit windows also cannot both be exercised in a fast test (the burst always trips first), so the sustained-limit test seeds the hourly window with timestamps older than the burst window to isolate it. The strict frontend CSP was the riskiest change, so before committing it the production build was scanned directly: no inline scripts, no external fetches, no web fonts, no service worker, confirming `script-src 'self'` would not break the app, with `'unsafe-inline'` permitted for styles only.

**Next:** If the backend ever scales past one instance, move the limiter and daily budget into Redis so the limits hold across the fleet; consider per-route budgets if scan and chat costs diverge.
